...
- 根据 all_acc 阈值筛选，输出结果到同目录下的 txt 报告（默认：results_grid_weight_scan.txt）
...
def score_with_weights(metrics: Dict[str, Any], weights: Tuple[int, int, int, int]) -> float:
    ...
    return (
        w_l1 * metrics.get("l1_loss", 0.0)
        + w_sep * metrics.get("separation_score", 0.0)
        + w_sil * metrics.get("silhouette", 0.0)
        + w_pen * metrics.get("penalty_score", 0.0)
    )
...
def main():
    ...
    for weights in enumerate_weights(args.weight_sum):
        for (k, dp), metrics in results.items():
            ...
            candidates.append((weights, (k, dp), metrics, new_score))
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        f.write("# 格式: weights(w_l1,w_sep,w_sil,w_pen) | k,dp | all/old/new_acc | score(orig) | score(weighted) | n_clusters | l1/separation/silhouette/penalty\n\n")
        for weights, (k, dp), m, new_score in candidates:
            line = (
                ...
                f"score_orig={m.get('score'):.4f} | score_weighted={new_score:.4f} | "
