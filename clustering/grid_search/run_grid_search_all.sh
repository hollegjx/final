...
for superclass in "${superclasses[@]}"; do
    ...
    # 运行网格搜索（使用clustering模块）
    echo "使用可配置参数范围..."

    # 构建基础命令
    cmd="python -m clustering.grid_search.grid_search \
...
echo "日志保存在: $log_file"
echo ""
echo "💡 要生成热力图，请运行: bash clustering/grid_search/generate_all_heatmaps.sh"
echo "=================================================="
