...
使用clustering/data增强型数据提供器，高效读取数据
"""
...
def test_adaptive_clustering_on_superclass(superclass_name, model_path,
                                         ...
                                         label_guide=False, analyze_errors=False,
    ...
    # 更新日志中的数据集名称（如果启用了detail_dense）
    if detail_dense and not silent:
        from ..information import get_logger
        logger = get_logger()
        if logger and logger.enabled:
            logger.update_dataset_name(superclass_name)
    ...
    if eval_dense:
        ...
        # ========== 步骤5.5: 错误样本分析（可选） ==========
        if analyze_errors and not silent and not eval_dense and error_count > 0:
            from ..information.error_sample_analysis import analyze_error_samples
            from ..density.density_estimation import (
                compute_simple_density, compute_median_density,
            ...
            from ..utils.co_calculation import compute_co_value

            # 重新计算密度和邻居信息（用于分析）
            if dense_method == 0:
                densities_for_analysis, knn_distances, neighbors_for_analysis = compute_simple_density(X, k=k)
            elif dense_method == 1:
                densities_for_analysis, knn_distances, neighbors_for_analysis = compute_median_density(X, k=k)
            elif dense_method == 2:
                densities_for_analysis, knn_distances, neighbors_for_analysis = compute_normalized_inverse_density(X, k=k)
            elif dense_method == 3:
                densities_for_analysis, knn_distances, neighbors_for_analysis = compute_exponential_density(X, k=k)
            else:
                densities_for_analysis, knn_distances, neighbors_for_analysis = compute_simple_density(X, k=k)

            # 计算高密度点掩码
            relative_densities = compute_relative_density(densities_for_analysis, neighbors_for_analysis, k)
            high_density_mask_base = identify_high_density_points(relative_densities, density_percentile, use_relative=True)

            # 计算co值
            co = compute_co_value(
                co_mode=co_mode, knn_distances=knn_distances, densities=densities_for_analysis,
                neighbors=neighbors_for_analysis, k=k, co_manual=co_manual, silent=True
            )

            # 构建high_density_neighbors_map（仅用于显示高密度子空间邻居）
            _, _, high_density_neighbors_map, _ = build_clusters_ssddbc(
                X, high_density_mask_base, neighbors_for_analysis, labeled_mask, targets,
                densities_for_analysis, known_mask, k=k, co=co, silent=True
            )

            # 如果启用label_guide，扩展高密度掩码（仅影响报告中的"是否高密度点"显示）
            if label_guide:
                # 复制高密度掩码
                extended_high_density_mask = high_density_mask_base.copy()

                # 找出稀疏点中的有标签样本
                sparse_mask = ~high_density_mask_base
                labeled_sparse_mask = sparse_mask & labeled_mask

                # 将这些有标签稀疏点也标记为高密度（扩展核心点空间）
                extended_high_density_mask[labeled_sparse_mask] = True

                high_density_mask_for_analysis = extended_high_density_mask
                core_space_description = "高密度点 + 有标签稀疏点 (label_guide=True)"
            else:
                high_density_mask_for_analysis = high_density_mask_base
                core_space_description = "仅高密度点 (label_guide=False)"

            # 执行错误样本分析（使用实际聚类的prototypes，确保簇ID和距离一致）
            analyze_error_samples(
                all_predictions=predictions,
                all_targets=targets,
                all_known_mask=known_mask,
                X_all=X,
                densities_all=densities_for_analysis,
                neighbors_all=neighbors_for_analysis,
                high_density_neighbors_map=high_density_neighbors_map,
                high_density_mask=high_density_mask_for_analysis,
                train_size=train_size,
                dataset_name=superclass_name,
                core_space_description=core_space_description,
                prototypes=prototypes  # 使用实际聚类返回的prototypes
            )
