...
from clustering.data import set_deterministic_behavior  # 从clustering/data导入
...
try:
    ...
    # 直接运行时的导入
    from clustering.ssddbc.analysis import analyze_cluster_composition
    from clustering.baseline.kmeans import test_kmeans_baseline
...
def main():
    ...
    parser.add_argument('--dense_method', type=int, default=0, choices=[0, 1, 2, 3],
                        help='密度计算方法，默认0。0=平均距离倒数(通用默认)，1=中位数距离倒数(抗噪声)，2=归一化倒数(密度值归一化到[0,1])，3=指数密度(强调局部密集度)。详见clustering/density/DENSITY_METHODS.md')

    # ========== 3. 骨干网络（高密度点）聚类相关参数 ==========
    parser.add_argument('--co_mode', type=int, default=2, choices=[1, 2, 3],
                        help='co截止距离计算模式，默认2。1=手动指定(需配合--co_manual)，2=K近邻平均距离(通用默认)，3=相对自适应距离(每点自适应，适合不均匀分布)。详见clustering/utils/CO_MODES.md')
    ...
    parser.add_argument('--assign_model', type=int, default=2, choices=[1, 2, 3],
                        help='低密度点分配策略，默认2。1=簇原型就近(速度最快)，2=KNN投票加权(推荐默认，考虑邻域信息)，3=簇内K近邻平均距离(最精细但慢)。详见clustering/ssddbc/ASSIGNMENT_STRATEGIES.md')
    ...
    parser.add_argument('--detail_dense', type=str2bool, default=False,
                        help=f'是否记录骨干网络聚类详细日志，默认False。True=将高密度点聚类详细信息保存到{clustering_log_dir}目录下，便于分析聚类过程')
    ...
    parser.add_argument('--analyze_errors', type=str2bool, default=False,
                        help=f'是否分析测试集错误样本，默认False。True=生成包含密度、标签、邻居等信息的报告并写入{error_analysis_log_dir}')
    ...
    try:
        # 网格搜索功能已迁移到 clustering.grid_search.batch_runner
        if args.grid_search:
            print("🔍 网格搜索功能已迁移到 clustering.grid_search.batch_runner")
            print("请使用: python -m clustering.grid_search.batch_runner")
            return
        ...
        ssddbc_results = test_adaptive_clustering_on_superclass(
            ...
            detail_dense=args.detail_dense,  # 传递骨干网络详细日志参数
            label_guide=args.label_guide,  # 传递标签引导模式参数
            analyze_errors=args.analyze_errors,  # 传递错误样本分析参数
