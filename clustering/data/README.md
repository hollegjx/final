# clustering/data - 独立数据读取模块

## 📋 概述

这是clustering模块的独立数据读取系统，提供特征加载和提取功能，**最小化对外部训练代码的依赖**。

### 设计目标

1. **独立性**: clustering模块应该能够独立运行，不过度依赖训练代码
...
**使用示例**:
```python
from clustering.data.dataset_config import get_superclass_info
...
**使用示例**:
```python
from clustering.data import FeatureLoader
...
**使用示例**:
```python
from clustering.data import FeatureExtractor
import torch
...
```bash
# 测试所有模块
python -m clustering.data.test_data_modules

# 只测试数据集配置模块
python -m clustering.data.test_data_modules --test config

# 只测试缓存加载器
python -m clustering.data.test_data_modules --test loader

# 只测试数据提供器
python -m clustering.data.test_data_modules --test provider

# 只测试特征提取器结构
python -m clustering.data.test_data_modules --test extractor
```

### 查看使用示例

```bash
python -m clustering.data.example_usage
```
