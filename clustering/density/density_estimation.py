...
def compute_normalized_inverse_density(X, k=10):
    """
    使用k近邻平均距离倒数计算密度，然后归一化到[0,1]区间

    公式: ρ(x_i) = (1/K * Σ d(x_i, x_j))^(-1)  其中 j ∈ KNN(i)
    然后归一化: ρ_norm = (ρ - ρ_min) / (ρ_max - ρ_min)
    ...
    Returns:
        densities: 归一化后的密度值 [0, 1]
        knn_distances: k近邻距离矩阵
        neighbors: k近邻索引矩阵
    """
    print(f"[DENSITY] 计算归一化倒数密度 (k={k})...")
    ...
    for i in range(len(neighbors)):
        ...
        row_neighbors = neighbors[i]
        dist_idx_pairs = list(zip(row_distances, row_neighbors))
        dist_idx_pairs.sort(key=lambda x: (x[0], x[1]))
        knn_distances[i] = np.array([pair[0] for pair in dist_idx_pairs])
        neighbors[i] = np.array([pair[1] for pair in dist_idx_pairs])
    ...
    print(f"   原始密度范围: [{rho_min:.3f}, {rho_max:.3f}]")
    print(f"   归一化后: [0.000, 1.000], mean={densities.mean():.3f}")

    return densities, knn_distances, neighbors


def compute_exponential_density(X, k=10):
    """
    使用指数函数计算密度
