...
def compute_total_loss(X, predictions, targets, labeled_mask,
    ...
    # 精简版：仅支持通过 l2_components 显式指定组件（如 ['separation', 'silhouette']），
    # 不再根据 use_cluster_quality 推导默认组件集合。
    if resolved_components is None:
        ...
        if use_cluster_quality:
            warnings.warn(
                "当前实现下，use_cluster_quality 与 l2_components 同时设置时，仅使用 l2_components。",
                RuntimeWarning
            )
