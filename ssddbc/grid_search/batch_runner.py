...
def _run_single_process(args_tuple):
    ...
    (superclass, model_path, k, density_percentile, eval_version, co_mode,
     ...
     l1_type, separation_weight, penalty_weight, l2_components, l2_component_weights,
     feature_cache_dir, run_kmeans) = args_tuple
...
def _run_parallel_grid_search(
    ...
    assign_model, voting_k, use_train_and_test, detail_dense,
    use_cluster_quality, cluster_distance_method, l1_type,
    separation_weight, penalty_weight, l2_components, l2_component_weights, feature_cache_dir, max_workers
):
    """并行版本的网格搜索"""
    total = len(k_values) * len(density_values)
    ...
    with output_file.open('w', encoding='utf-8') as handle:
        ...
        handle.write(f"feature_cache_dir: {feature_cache_dir}\n")
...
def _run_serial_grid_search(
    ...
    assign_model, voting_k, use_train_and_test, detail_dense,
    use_cluster_quality, cluster_distance_method, l1_type,
    separation_weight, penalty_weight, l2_components, l2_component_weights, feature_cache_dir
):
    """串行版本的网格搜索（原实现）"""
    total = len(k_values) * len(density_values)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Python-based batch grid search runner.',
    ...
    parser.add_argument('--feature_cache_dir', type=str, default=None,
                        help='自定义特征缓存根目录，程序会在该目录下按超类文件夹查找缓存（默认读取 config.py 的 FEATURE_CACHE_DIR）。')
    ...
    # 与当前实验强绑定的核心超参数：仅保留你关心的这几项
    parser.add_argument('--dense_method', type=int, default=1, choices=[1],
                        help='密集样本选择方法：当前脚本只支持 1=中位数密度。')
    parser.add_argument('--co_mode', type=int, default=2, choices=[2],
                        help='co 截止模式：当前脚本只支持 2=K近邻平均距离。')
    parser.add_argument('--assign_model', type=int, default=3, choices=[3],
                        help='稀疏点分配模型：当前脚本只支持 3=簇内K近邻平均距离。')

    # 聚类质量评估相关（固定使用 cluster_quality + 原型距离）
    parser.add_argument('--use_cluster_quality', action='store_true', default=False,
                        help='是否使用聚类质量评估指标作为L2损失（当前脚本要求开启）。')
    parser.add_argument('--cluster_distance_method', type=int, default=3, choices=[3],
                        help='簇距离计算方法：当前脚本只支持 3=原型距离。')
    parser.add_argument('--l2_components', type=str, default=None,
                        help='指定要计算的 L2 组件（当前脚本只支持 "separation silhouette"）。')
    ...
    # ================= 固定实验配置的安全校验 =================
    # 这里显式限制脚本只在你指定的配置空间内工作，
    # 确保后续删减其它分支后，行为仍与当前实验完全一致。

    # 1) 密度计算方法：仅支持中位数密度
    if args.dense_method != 1:
        raise ValueError("当前批量网格搜索脚本仅支持 --dense_method 1（中位数密度）。")

    # 2) co 计算模式：仅支持全局 KNN 平均距离
    if args.co_mode != 2:
        raise ValueError("当前批量网格搜索脚本仅支持 --co_mode 2。")

    # 3) 稀疏点分配策略：仅支持簇内K近邻平均距离
    if args.assign_model != 3:
        raise ValueError("当前批量网格搜索脚本仅支持 --assign_model 3。")

    # 4) 聚类质量评估：必须启用，并且仅支持原型距离
    if not args.use_cluster_quality:
        raise ValueError("当前批量网格搜索脚本要求始终开启 --use_cluster_quality。")
    if args.cluster_distance_method != 3:
        raise ValueError("当前批量网格搜索脚本仅支持 --cluster_distance_method 3（原型距离）。")

    # 5) L2 组件：仅支持 separation + silhouette，且不允许额外组件
    expected_components = {"separation", "silhouette"}
    if parsed_l2_components is None:
        raise ValueError('当前脚本需要显式指定 --l2_components "separation silhouette"。')
    if set(parsed_l2_components) != expected_components:
        raise ValueError('当前脚本仅支持 --l2_components "separation silhouette"（不允许额外组件）。')
    ...
    # 未再暴露给命令行的聚类配置保持为固定值，以保证行为与原始实验一致
    eval_version = 'v2'
    co_manual = None
    use_l2 = True
    eval_dense = False
