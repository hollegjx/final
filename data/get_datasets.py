...
from data.cifar100_superclass import get_cifar100_superclass_datasets, get_single_superclass_datasets, get_superclass_splits
...
from data.cifar100_superclass import subsample_classes as subsample_dataset_cifar100_superclass
...
sub_sample_class_funcs = {
    ...
    'cifar100_superclass': subsample_dataset_cifar100_superclass,
...
get_dataset_funcs = {
    ...
    'cifar100_superclass': get_cifar100_superclass_datasets,
    ...
    for dataset_name, dataset in datasets.items():
        if dataset is not None:
            ...
            if hasattr(dataset, 'label_mapping') and dataset.label_mapping is not None:
                # 超类数据集已经有了自己的标签映射，不覆盖
                print(f"检测到超类数据集 {dataset_name}，保持现有标签映射")
            else:
                # 普通数据集，应用标准的target_transform
...
def get_class_splits(args):
    ...
    quiet = getattr(args, 'is_grid_search', False)
    ...
    if args.dataset_name == 'cifar10':
    ...
    elif args.dataset_name == 'cifar100_superclass':
        ...
        if hasattr(args, 'superclass_name') and args.superclass_name:
            ...
            if args.superclass_name in superclass_splits:
                split_info = superclass_splits[args.superclass_name]
                args.train_classes = split_info['known_classes']
                args.unlabeled_classes = split_info['unknown_classes']
                if not quiet:
                    print(f"使用超类 '{args.superclass_name}' 的划分:")
                    print(f"  已知类: {args.train_classes}")
                    print(f"  未知类: {args.unlabeled_classes}")
            else:
                raise ValueError(f"未知的超类名称: {args.superclass_name}")
        else:
            # 默认使用标准CIFAR-100划分
            args.train_classes = range(80)
            args.unlabeled_classes = range(80, 100)
            if not quiet:
                print("使用标准CIFAR-100划分 (如需使用特定超类，请设置 args.superclass_name)")
